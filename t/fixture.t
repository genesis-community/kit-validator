...
use_ok 'Genesis::Kit::Validator::Fixture';
...
subtest 'constructor requires kit_dir' => sub {
	eval { Genesis::Kit::Validator::Fixture->new() };
	like $@, qr/kit_dir is required/, 'no args -> die';

	eval { Genesis::Kit::Validator::Fixture->new(kit_dir => '/nonexistent-xyzzy-42') };
	like $@, qr/kit_dir does not exist/, 'bad path -> die';
};

subtest 'path resolves category + name to spec/<cat>/<name>.yml' => sub {
	my $fx = Genesis::Kit::Validator::Fixture->new(kit_dir => $kit);
