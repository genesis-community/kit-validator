...
use_ok 'Genesis::Kit::Validator::Prune';

# Genesis::Kit::Validator::Prune::prune_manifest strips the same keys the Go
...
subtest 'always drops top-level meta/pipeline/params/kit/genesis/compilation' => sub {
	my $m = base_manifest();
	my ($pruned, $bosh_vars) = Genesis::Kit::Validator::Prune::prune_manifest($m, {is_proto => 0});
...
subtest 'non-proto env: also drops network-shape top-level keys' => sub {
	...
	my ($pruned) = Genesis::Kit::Validator::Prune::prune_manifest($m, {is_proto => 0});
...
subtest 'proto env: retains network-shape top-level keys' => sub {
	...
	my ($pruned) = Genesis::Kit::Validator::Prune::prune_manifest($m, {is_proto => 1});
...
subtest 'input is not mutated' => sub {
	...
	Genesis::Kit::Validator::Prune::prune_manifest($m, {is_proto => 0});
